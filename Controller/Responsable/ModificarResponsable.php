...
<body>
    ...
    if (isset($_POST['btnModificar'])) {
        if (
            ...
            try {
                $sql = "UPDATE Responsable
                        SET Nombres=?, Apellidos=?, Telefono=?, Correo=?, DNI=?, CodigoAmbiente=?
                        WHERE Codigo=?";
                $sentencia = $conexion->prepare($sql);
                $sentencia->execute([$nombres, $apellidos, $telefono, $correo, $DNI, $codigoAmbiente, $codigo]);
    
                echo "Responsable modificado";
                echo "<a href='MostrarResponsable.php' class='btn btn-info'>Regresar a mantenimiento</a>";
        ...
        $sql = "SELECT * FROM Responsable WHERE Codigo = $codigo";
        ...
        <div class="container">
            ...
            <form action="ModificarResponsable.php" method="POST">
                ...
                <select name="CodigoAmbiente" id="CodigoAmbiente" class="form-control" required>
                    <option value="">Seleccione el nuevo ambiente del responsable</option>
                    <?php
